...
const dangerStats = [
  {
    ...
    detail: "Infertile women got 31% of their calories from UPF vs. much less in fertile women. Every extra 10% makes it worse.",
  ...
  },
  {
    ...
    detail: "These metals build up silently from food, water, cosmetics, and the environment — and studies show infertile women often have significantly higher levels.",
...
const hopeStats = [
  {
    ...
    source: "Source: Lifestyle intervention RCT in infertile women, Human Reproduction / Frontiers in Endocrinology, 2024",
  },
  {
    number: "nearly 3x",
    ...
    <section id="fertility-crisis" className={`bg-background ${dramatic ? "py-20" : "py-14"}`}>
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-4xl text-center">
          <h2 className={`${dramatic ? "text-4xl lg:text-6xl" : "text-3xl lg:text-4xl"} font-bold leading-tight text-charcoal`}>
          ...
          <p className="mx-auto mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground lg:text-lg">
            These everyday exposures are slashing your chances of building a tiny human… and almost no one is talking about them.
          </p>
        </div>
        ...
        <div className={`${dramatic ? "mt-14 gap-8" : "mt-10 gap-6"} grid lg:grid-cols-2`}>
          {dangerStats.map((stat, index) => (
            <div key={stat.title} className="rounded-2xl border border-destructive/25 bg-card p-8 shadow-lg lg:p-10">
              ...
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground lg:text-base">{stat.body}</p>
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground lg:text-base">{stat.detail}</p>
              <p className="mt-5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{stat.source}</p>
            </div>
          ))}
        </div>
        ...
        <div className={`${dramatic ? "mt-16" : "mt-12"} mx-auto max-w-4xl rounded-2xl border border-border bg-rose-accent p-8 text-center shadow-xl lg:p-10`}>
          <h3 className="text-2xl font-bold text-charcoal lg:text-4xl">Ready to Reverse the Damage and Reclaim Your Fertility?</h3>
